...
"""Install Claude Code global assets on Linux with Python 2.7."""
...
def get_template_root():
    ...
    Returns:
        str: Absolute repository root containing the Claude template sources.
...
def validate_sources(template_root):
    ...
    if missing_paths:
        raise RuntimeError(
            "Claude template source is missing: {0}".format(", ".join(missing_paths))
        )
...
def install_global_claude(
    ...
    Examples:
        ...
        >>> install_global_claude("/template", "/tmp/home")  # doctest: +SKIP
        True
    """
    validate_platform()
    ...
    if installed:
        output_function("Installed Claude global template into {0}".format(claude_root))
    return installed
